...
const sanitizeCode = (code) => code
    ...
    .replace(/```$/g, "")
    .trim();
const enhanceProjectPrompt = async (message) => {
...
const generateProjectCode = async (currentCode, enhancedPrompt) => {
...
export const makeRevision = async (req, res) => {
    ...
    try {
        ...
        const code = await generateProjectCode(currentProject.current_code || "", enhancedPrompt);
...
export const rebuildProject = async (req, res) => {
    const userId = req.userId;
    try {
        const projectId = getParamValue(req.params.projectId);
        if (!userId) {
        ...
        await prisma.conversation.create({
            data: {
                role: "assistant",
                content: "I've rebuilt your website! You can now preview it and continue editing.",
                projectId,
            },
        });
        await prisma.websiteProject.update({
